...
function resetKamera(tipe = 'absen', btnRef = null) {
    fotoData = null;
    if (tipe === 'absen') {
        document.getElementById('video-feed').classList.remove('hidden');
        document.getElementById('img-final').classList.add('hidden');
        document.getElementById('btn-ambil').classList.remove('hidden');
        document.getElementById('btn-ulang').classList.add('hidden');
    } else {
        document.getElementById('video-setor').classList.remove('hidden');
        document.getElementById('img-setor-final').classList.add('hidden');
        btnRef.innerText = "AMBIL BUKTI SETOR";
        btnRef.onclick = () => tangkapFoto('setor');
        btnRef.classList.replace('bg-amber-500', 'bg-indigo-600');
    }
}
...
async function prosesAbsen(shiftId) {
    ...
    try {
        // 1. Ambil setting jam masuk untuk shift ini dari database
        ...
        const totalMenitJadwal = (shiftSetting.jam_masuk * 60) + shiftSetting.minit_masuk; // Pastikan kolom ini 'menit_masuk' di DB
        ...
        // 3. Logika Penentuan Poin (Sesuai Permintaan)
        if (selisih <= 0) {
            poinDapat = 20; // Tepat waktu / Lebih awal
            statusAbsen = "Tepat Waktu";
        } else if (selisih > 0 && selisih <= 15) {
            poinDapat = 10; // Telat <= 15 menit
            statusAbsen = "Telat < 15 Menit";
        } else {
            poinDapat = 5;  // Telat > 15 menit
            statusAbsen = "Telat > 15 Menit";
        }

        // 4. Simpan ke tabel presensi menggunakan kolom 'point_didapat'
        const { error: errPresensi } = await supabaseClient
            ...
            .insert([{
                ...
                poin_didapat: poinDapat, // Nama kolom disesuaikan
                keterangan: statusAbsen
            }]);

        if (errPresensi) throw errPresensi;

        // 5. Update Total Score di tabel users (Akumulasi Poin)
        ...
        const scoreBaru = (userData.score || 0) + poinDapat;
        ...
        console.error(err);
        alert("Gagal memproses poin: " + err.message);
    }
}
...
// --- FITUR 2: RANKING (FIXED LOGIC) ---
async function updateLeaderboard(tipe) {
    const now = new Date();
    let start, end;
    // Tambahkan potongan ini di fungsi updateLeaderboard() bagian render HTML
if (tipe === 'bulan' && sorted.length >= 3) {
   let podiumHTML=`
<div class="flex items-end justify-center gap-3 mb-10 mt-6 h-56">
    ...
    <div class="flex flex-col items-center">
        ...
        <div class="relative mb-3">
            ...
            <div class="relative w-16 h-16 rounded-full border-[4px] border-slate-300 overflow-hidden bg-white shadow-xl">
                <img
                    src="https://ui-avatars.com/api/?name=${sorted[1][0]}&background=cbd5e1&color=fff"
        ...
        <div class="relative bg-gradient-to-b from-slate-200 to-slate-400 w-24 h-28 rounded-t-[24px] flex flex-col items-center justify-center shadow-2xl border-4 border-slate-300">
            ...
            <div class="absolute -top-4 bg-white border-4 border-slate-300 w-10 h-10 rounded-full flex items-center justify-center shadow-lg">
                🥈
            </div>

            <span class="text-white font-black text-3xl mt-2">
                2
            </span>

            <span class="text-[10px] text-white font-bold truncate px-2 max-w-full">
                ${sorted[1][0]}
            </span>
    ...
    <div class="flex flex-col items-center">
        ...
        <div class="relative mb-3">
            ...
            <div class="relative w-20 h-20 rounded-full border-[5px] border-yellow-400 overflow-hidden bg-white shadow-2xl">
                <img
                    src="https://ui-avatars.com/api/?name=${sorted[0][0]}&background=facc15&color=fff"
                    class="w-full h-full object-cover"
                >
            </div>
        ...
        <div class="relative bg-gradient-to-b from-yellow-300 to-yellow-500 w-28 h-40 rounded-t-[28px] flex flex-col items-center justify-center shadow-2xl border-[5px] border-yellow-400">
            ...
            <div class="absolute -top-5 bg-white border-[5px] border-yellow-400 w-14 h-14 rounded-full flex items-center justify-center shadow-xl text-2xl">
                👑
            </div>

            <span class="text-white font-black text-5xl mt-4 drop-shadow-lg">
                1
            </span>

            <span class="text-sm text-yellow-50 font-extrabold truncate px-2 max-w-full">
                ${sorted[0][0]}
            </span>
    ...
    <div class="flex flex-col items-center">
        ...
        <div class="relative mb-3">
            ...
            <div class="relative w-16 h-16 rounded-full border-[4px] border-orange-400 overflow-hidden bg-white shadow-xl">
                <img
                    src="https://ui-avatars.com/api/?name=${sorted[2][0]}&background=fb923c&color=fff"
                    class="w-full h-full object-cover"
                >
            </div>
        </div>
        ...
        <div class="relative bg-gradient-to-b from-orange-300 to-orange-500 w-24 h-24 rounded-t-[24px] flex flex-col items-center justify-center shadow-2xl border-4 border-orange-400">
            ...
            <div class="absolute -top-4 bg-white border-4 border-orange-400 w-10 h-10 rounded-full flex items-center justify-center shadow-lg">
                🥉
            </div>
            ...
            <span class="text-[10px] text-orange-50 font-bold truncate px-2 max-w-full">
                ${sorted[2][0]}
            </span>
    ...
    try {
        ...
        if(sorted.length === 0) {
            ...
            listEl.innerHTML = sorted.map((u, i) => `
                ...
                <div onclick="ambilUserIdLaluRiwayat('${u[0]}')" class="flex justify-between items-center p-4 bg-white rounded-2xl border cursor-pointer hover:bg-slate-50 transition-all">
